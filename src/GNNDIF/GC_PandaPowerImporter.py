# Improved GC_PandaPowerImporter.py
# Specialized fix: Attribute name case sensitivity causing "fake write" success
import numpy as np
...
def _set_val(obj, attr, value):
    """Simple brute-force value assignment, prints on failure"""
    try:
        setattr(obj, attr, value)
    except Exception as e:
        # print(f"[Warn] Cannot set {attr}: {e}")
        pass


def _get_iterable(obj, attr_list):
    """Retrieve iterable object using multiple possible attribute names"""
...
def _deactivate(obj):
    """Deactivate grid component by setting various status attributes to False"""
...
def remove_islands(grid):
    """
    Remove isolated islands from the grid, keeping only the main connected component.
    
    Island detection algorithm:
    1. Identify slack/reference buses as starting points
    2. Perform BFS to find all buses connected to slack buses
    3. Deactivate components not in the main connected island
    
    Args:
        grid: GridCal MultiCircuit object
    """
    ...
    # Handle different grid representations
    if not branches:
    ...
    # Identify slack/reference buses (starting points for island detection)
    slack_buses = set()
    for g in generators:
        is_slack = False
    ...
    if not slack_buses:
        return  # No slack buses found, cannot determine main island

    # Build adjacency list for buses
    ...
    # BFS to find main connected island starting from slack buses
    ...
    # Deactivate components not in the main island
    deactivated_count = 0
    for br in branches:
        f = getattr(br, 'bus_from', getattr(br, 'from_node', getattr(br, 'busFrom', None)))
        t = getattr(br, 'bus_to', getattr(br, 'to_node', getattr(br, 'busTo', None)))
    ...
    for g in generators:
        ...
        if b and b not in main_island_buses: 
            _deactivate(g)
    ...
    print(f"[Cleaner] Found {len(buses) - len(main_island_buses)} island buses, deactivated {deactivated_count} associated branches.")


def PP2GC(net_pp):
    """
    Convert PandaPower network to GridCal MultiCircuit.
    
    Handles key conversions:
    - Bus voltage levels and limits
    - Loads, generators, and renewable sources
    - Lines and transformers with impedance calculations
    - Switches as low-impedance branches
    - Island detection and cleanup
    
    Args:
        net_pp: PandaPower network object
        
    Returns:
        GridCal MultiCircuit object
    """
    grid = gce.MultiCircuit()
    name = str(net_pp.name) if hasattr(net_pp, 'name') and net_pp.name else "SimBench_Import"
    _set_val(grid, 'name', name)

    # Set system base power
    base_mva = float(net_pp.sn_mva)
    _set_val(grid, 'Sbase', base_mva)

    bus_map = {}  # Map pandapower bus indices to GridCal bus objects
    ...
    for idx, row in net_pp.bus.iterrows():
        ...
        # IMPORTANT: GridCal uses uppercase 'Vnom' for nominal voltage
        ...
        # Initial voltage for power flow
    ...
    # --- Renewable Generator (Sgen) Conversion ---
    if not net_pp.sgen.empty:
        for idx, row in net_pp.sgen.iterrows():
            ...
            # Determine maximum available power
            sn = float(row.get('sn_mva', 0.0))
            p_max_val = sn if sn > 0 else float(row['p_mw'])

            # IMPORTANT: GridCal uses uppercase 'Pmax'
            _set_val(g, 'Pmax', p_max_val)
            _set_val(g, 'Pmin', 0.0)
            _set_val(g, 'is_controlled', True)
            _set_val(g, 'is_slack', False)
            _set_val(g, 'Cost2', 1.0)  # Linear cost coefficient
            _set_val(g, 'Cost1', 0.0)  # Quadratic cost coefficient
    ...
    # --- External Grid (Slack Bus) Conversion ---
    if not net_pp.ext_grid.empty:
        for idx, row in net_pp.ext_grid.iterrows():
            ...
            _set_val(g, 'Pmax', 99999.0)   # Unlimited capacity
            _set_val(g, 'Pmin', -99999.0)  # Can absorb power
            _set_val(g, 'Cost2', 100.0)    # High cost for slack bus
    ...
    if not net_pp.line.empty:
        for idx, row in net_pp.line.iterrows():
            ...
            if f_bus and t_bus:
                ...
                # Ensure minimum impedance values for numerical stability
                if abs(r_pu) < 1e-12: r_pu = MIN_Z_LINE
                if abs(x_pu) < 1e-12: x_pu = MIN_Z_LINE

                # IMPORTANT: GridCal uses uppercase 'R', 'X', 'B'
                _set_val(br, 'R', r_pu)
                _set_val(br, 'X', x_pu)
                _set_val(br, 'B', b_pu)
                _set_val(br, 'rate', rate_mva)
    ...
    if not net_pp.trafo.empty:
        for idx, row in net_pp.trafo.iterrows():
            ...
            if hv_bus and lv_bus:
                ...
                # Calculate transformer impedance from percentage values
                sn_mva = row['sn_mva']
                vn_hv = row['vn_hv_kv']
                vk_percent = row['vk_percent']      # Short-circuit voltage %
                vkr_percent = row['vkr_percent']    # Copper losses %
                ...
                # IMPORTANT: GridCal uses uppercase 'R', 'X'
                _set_val(t, 'R', r_pu)
                _set_val(t, 'X', x_pu)
                _set_val(t, 'rate', sn_mva)
                _set_val(t, 'tap_module', 1.0)  # Fixed tap setting
                _set_val(t, 'active', True)

                grid.add_branch(t)

    # --- Switch Conversion ---
    # Convert closed bus-bus switches to low-impedance branches
    if not net_pp.switch.empty:
        for idx, row in net_pp.switch.iterrows():
            if not row.get('closed', True): 
                continue
            if row['et'] != 'b':  # Only handle bus-bus switches (not line switches)
                continue
            ...
            if bus_id in bus_map and element_id in bus_map:
                ...
                # Switches modeled as near-zero impedance branches
                _set_val(sw_br, 'R', EPSILON)
                _set_val(sw_br, 'X', EPSILON)
                _set_val(sw_br, 'rate', 9999.0)  # High thermal limit
                _set_val(sw_br, 'active', True)

                grid.add_branch(sw_br)

    # Post-processing: Clean up isolated islands
    print("[Importer] Checking and cleaning islands...")
    remove_islands(grid)

    # Compile grid for GridCal engine
    print("[Importer] Compiling GridCal network...")
    try:
        grid.compile()
    except Exception as e:
        # Some GridCal versions might throw errors during compile
        pass

    print(f"[Importer] Successfully converted SimBench network: {net_pp.name}")
    return grid